"""Trade-CSVs der besten (nicht ueberlebenden) Kandidaten: run() identisch zu scan_exec.py."""
...
D = "/tmp/claude-0/-home-user-Florian/154c19e6-740a-5434-82b2-7192601fe205/scratchpad/research/r4/tf5_exec/"
for tag, name, tf, body, mode, fn in (("NQ", "London 02-05", 5, 0.6, "confirm", "C1_NQ_London_tf5_b0.6_confirm.csv"),
                                       ("YM", "Asia 18-02", 5, 0.6, "base", "C2_YM_Asia_tf5_b0.6_base.csv"),
                                       ("YM", "Asia 18-02", 1, 0.6, "base", "C3_YM_Asia_tf1_b0.6_base.csv"),
                                       ("NQ", "London 02-05", 15, 0.6, "base", "C4_NQ_London_tf15_b0.6_base.csv")):
    if mk is None or mk.tag != tag: mk = Market(tag)
    rows = run(name, tf, body, mode); save_csv(rows, D + fn); print(fmt(f"{tag} {name} tf{tf} b{body} {mode}", rows)); print("  ->", D + fn)
